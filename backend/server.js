...
// Basic liveness probe (for ALB target group / ECS health checks)
app.get("/health", (req, res) => {
  // Report the process is up and how long it has been running
  return res.status(200).json({ status: "ok", service: "backend", uptime: process.uptime() });
});

// Database readiness probe — confirms the pool can reach MySQL
app.get("/health/db", async (req, res) => {
  try {
    // Cheap query that succeeds only if the DB connection is alive
    const [rows] = await db.query("SELECT 1 AS db_up");
    // DB reachable — report connected status
    return res.status(200).json({ status: "ok", database: "connected", result: rows[0] });
  } catch (error) {
    // Query failed — the database is unreachable; log and report down
    console.error("❌ DB health check failed:", error.message);
    return res.status(500).json({ status: "error", database: "down", error: error.message });
  }
});
